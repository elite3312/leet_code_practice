    ...
    def minFallingPathSum(self, matrix: list[list[int]]) -> int:
        ...
        def recurse(r,c):
            ...
            if r < 0 or c < 0 or r > row-1 or c > col-1:
                return float('inf')
            if r == row-1: # last row reached - does not make sense to memoize here
                return grid[r][c]
            if (r,c) in memo:
                return memo[(r,c)]
            result = grid[r][c] + min(recurse(r+1, c), recurse(r+1, c+1), recurse(r+1, c-1)) 
            memo[(r,c)] = result
            #print('memo = ', memo)
            return result
